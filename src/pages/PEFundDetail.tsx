...
import { ArrowLeft, MapPin, TrendingUp, Users, FileText, Building2, ExternalLink, Linkedin, Calendar, Target, Globe, Briefcase } from "lucide-react";
...
import { usePEFund } from "@/hooks/usePEFunds";
...
const strategyColors: Record<string, string> = {
  buyout: "bg-blue-600/10 text-blue-600",
  growth: "bg-emerald-500/10 text-emerald-500",
  turnaround: "bg-orange-500/10 text-orange-500",
  distressed: "bg-red-500/10 text-red-500",
  secondaries: "bg-purple-500/10 text-purple-500",
  fund_of_funds: "bg-teal-500/10 text-teal-500",
  real_assets: "bg-amber-700/10 text-amber-700",
...
export default function PEFundDetail() {
  ...
  const { fund, loading, error } = usePEFund(id);
    ...
    <div className="min-h-screen bg-background pb-20">
      ...
      <div className="relative h-64 md:h-80 w-full overflow-hidden">
        ...
        <div className="absolute bottom-4 left-4 right-4 text-white">
          <div className="flex items-center gap-2 mb-2">
            {fund.strategy && (
              <Badge className={`${strategyColors[fund.strategy]} border-0`}>
      ...
      <Tabs defaultValue="overview" className="px-4 mt-4">
        ...
        <TabsContent value="overview" className="space-y-4 mt-4">
            ...
            <Card>
              ...
              <CardContent>
                <div className="flex flex-wrap gap-2">
                  {fund.target_sectors.map((sector, idx) => (
            ...
            <CardContent>
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  ...
                  <p className="font-medium text-emerald-500">{fund.preferred_return}%</p>
                </div>
                <div className="space-y-1">
                ...
                <div className="space-y-1">
                  ...
                  <p className="font-medium">{fund.investment_period_years} years</p>
                </div>
                <div className="space-y-1">
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-base">Investment Profile</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <p className="text-xs text-muted-foreground">Target Company Size</p>
                  <p className="font-medium">{fund.target_company_size || "Varies"}</p>
                </div>
                <div className="space-y-1">
                  <p className="text-xs text-muted-foreground">Geographic Focus</p>
                  <div className="flex items-center gap-1">
                    <Globe className="w-3 h-3 text-muted-foreground" />
                    <p className="font-medium">{fund.geographic_focus || "Global"}</p>
                  </div>
                </div>
            ...
            <>
                  ...
                  <CardContent className="p-4">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                      <div className="bg-emerald-500/10 rounded-lg p-3 text-center">
                        <p className="text-xl font-bold text-emerald-500">{record.tvpi}x</p>
                        <p className="text-xs text-muted-foreground">TVPI</p>
                      </div>
                      <div className="bg-blue-500/10 rounded-lg p-3 text-center">
                        <p className="text-xl font-bold text-blue-500">{record.irr}%</p>
                        <p className="text-xs text-muted-foreground">Net IRR</p>
                      </div>
                      {record.moic && (
                        <div className="bg-purple-500/10 rounded-lg p-3 text-center">
                          <p className="text-xl font-bold text-purple-500">{record.moic}x</p>
                          <p className="text-xs text-muted-foreground">MOIC</p>
                        </div>
                      )}
                      {record.dpi && (
                        <div className="bg-amber-500/10 rounded-lg p-3 text-center">
                          <p className="text-xl font-bold text-amber-500">{record.dpi}x</p>
              ...
              <Card className="bg-muted/50">
                <CardContent className="p-4 text-sm text-muted-foreground">
                  <p><strong>Note:</strong> Past performance is not indicative of future results. TVPI = Total Value to Paid-In. IRR = Internal Rate of Return. MOIC = Multiple on Invested Capital. DPI = Distributions to Paid-In.</p>
                </CardContent>
              </Card>
            </>
          ) : (
          ...
          {fund.portfolio_companies.length > 0 ? (
              ...
              <Card key={index}>
                <CardContent className="p-4 flex gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    {company.acquisition_date && (
                      <p className="text-xs text-muted-foreground mt-1">Acquired: {company.acquisition_date}</p>
                    )}
                    {company.website_url && (
            ...
            <Card>
              <CardContent className="p-8 text-center">
                ...
                <p className="text-sm text-muted-foreground mt-1">Investment period: {fund.investment_period_years} years</p>
              </CardContent>
            </Card>
          )}
        </TabsContent>
